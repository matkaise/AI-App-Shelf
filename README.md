...
Das Bundler-MVP unterstützt Single-File-React-/TSX-Apps mit öffentlichen Browser-tauglichen npm-Paketen. Direkte URL-Imports wie `import "https://..."` werden aus Sicherheitsgründen blockiert; npm-Pakete laufen nur über die kontrollierte `esm.sh`-Route. Lokale Projekt-Imports wie `@/components/ui/button`, Node-only-Pakete, native Addons und Multi-File-Projekte werden nicht automatisch aufgelöst.
